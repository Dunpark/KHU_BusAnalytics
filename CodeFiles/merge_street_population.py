# -*- coding: utf-8 -*-
"""
기존 통합 데이터에 길단위인구-상권 데이터를 추가로 병합하는 코드
"""
...
df_merged = pd.read_csv(
    'Merged_datasets/4개년_통합데이터_추정매출_상주인구_소득소비.csv',
    encoding='utf-8-sig',
    low_memory=False
)
...
try:
    df_street = pd.read_csv(
        'Data_Raw_new/서울시 상권분석서비스(길단위인구-상권).csv',
        encoding='utf-8-sig',
        low_memory=False
    )
...
print(f"길단위인구-상권 로드 완료: {df_street.shape}")

# 원본 데이터 행 수 (비교용)
ORIGINAL_ROWS = 290416  # 4개년추정매출_all_quarters.csv
...
# 서비스_업종_코드 유무 확인
if "서비스_업종_코드" in df_street.columns:
...
# =============================================================================
# 3. 길단위인구-상권 데이터 Merge (LEFT JOIN)
# =============================================================================
print("\n" + "=" * 70)
print("[3/5] 길단위인구-상권 데이터 Merge")
print("=" * 70)

# 중복 컬럼 제거
street_cols_to_drop = [col for col in df_street.columns 
                       if col not in join_keys and col in df_merged.columns]
print(f"제거할 중복 컬럼: {street_cols_to_drop}")

df_street_clean = df_street.drop(columns=street_cols_to_drop, errors='ignore')

# 길단위인구 관련 컬럼 리스트
street_data_cols = [col for col in df_street_clean.columns if col not in join_keys]
print(f"길단위인구 관련 컬럼 수: {len(street_data_cols)}개")
print(f"길단위인구 관련 컬럼: {street_data_cols[:10]}...")
...
# 결측치 확인 대상: 길단위인구 관련 컬럼
check_cols = [col for col in df_tmp.columns if col in street_data_cols]
...
output_path = 'Merged_datasets/4개년_통합데이터_추정매출_상주인구_소득소비_길단위인구.csv'
...
print(f"[최종] 4개년_통합데이터_추정매출_상주인구_소득소비_길단위인구.csv: {df_final.shape[0]:,}행")
print(f"\n총 누락된 행: {total_lost:,}행")
print(f"원본 대비 감소율: {total_lost/ORIGINAL_ROWS*100:.2f}%")
print(f"원본 대비 유지율: {df_final.shape[0]/ORIGINAL_ROWS*100:.2f}%")
